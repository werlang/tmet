...
- `SUAP_SELECTORS_FILE` (optional custom selector JSON path)

## SUAP selector configuration

SUAP login/session selectors are built into `config/suap-selectors.js`.

- Built-in defaults cover login, error messages, and session markers.
- If you need custom overrides per environment, point `SUAP_SELECTORS_FILE` to a custom JSON file.
- The scraper automatically merges custom overrides with safe built-in defaults.
